#!/usr/bin/env python3
"""Generate a landscape 2-column The Linux Survival Kit  —  EdenX Labs PDF."""
...
MARGIN = 1.0 * cm
GUTTER = 0.6 * cm  # space between columns
COL_W = (PAGE_W - 2 * MARGIN - GUTTER) / 2

# ── Styles ──────────────────────────────────────────────────────────
style_section = ParagraphStyle(
    "SectionTitle", fontName="Helvetica-Bold", fontSize=9.5,
    textColor=WHITE, leading=14, spaceBefore=0, spaceAfter=0,
    alignment=TA_LEFT, leftIndent=4,
)
style_cmd = ParagraphStyle(
    "Cmd", fontName="Courier-Bold", fontSize=6.5, textColor=HexColor("#0a58ca"),
    leading=9, spaceBefore=0, spaceAfter=0,
)
style_desc = ParagraphStyle(
    "Desc", fontName="Helvetica", fontSize=6.5, textColor=TEXT_MED,
    leading=9, spaceBefore=0, spaceAfter=0,
)
style_subtitle = ParagraphStyle(
    "Subtitle", fontName="Helvetica-Bold", fontSize=7.5, textColor=TEXT_DARK,
    leading=10, spaceBefore=4, spaceAfter=1,
)

# ── Data (same as portrait version) ────────────────────────────────
sections = [
    ...
    ("6 — FILE PERMISSIONS & OWNERSHIP", [
        ...
        ("umask 022", "Set default perms for new files (results in 644/755)"),
    ...
    ("7 — PROCESS MANAGEMENT", [
        ...
        ("Ctrl+Z  then  bg", "Suspend running process, continue in background"),
    ...
    ("21 — REDIRECTION PATTERNS & FD TRICKS", [
        ...
        ("echo 'msg' >&2", "Write to stderr (useful inside scripts)"),
        ("echo 'msg' > /dev/stderr", "Alternative: write to stderr via device path"),
        ("read line < /dev/stdin", "Explicitly read from stdin"),
        ("cat < /dev/null > empty.txt", "Create empty file using special devices"),
    ...
    ("22 — COMMAND CHAINING & PIPELINES", [
        ...
        ("cmd | head -1 | xargs -I {} sh -c 'echo {}'", "Get first line, use in another cmd"),
    ...
    ("24 — LOG ANALYSIS & FILTERING", [
        ...
        ("grep 'ERROR' app.log | awk '{print $1, $2}' | sort | uniq -c | sort -rn",
         "Rank error frequency by timestamp"),
        ("awk '/2025-03-26/ && /ERROR/' app.log", "Errors on a specific date"),
        ("awk '$9 == 500' access.log", "Find HTTP 500 responses in access log"),
    ...
    ("25 — CURL, REQUESTS & API FILTERING", [
        ...
        ("curl -s url | jq '.results[].name'", "Extract 'name' from each result item"),
        ("curl -s url | jq '.[] | select(.status==\"active\")'",
         "Filter JSON: only objects where status is active"),
    ...
    ("26 — PRACTICAL ONE-LINER RECIPES", [
        ...
        ("diff <(ssh host1 cat /etc/conf) <(ssh host2 cat /etc/conf)",
         "Compare a config across two remote servers"),
        ...
        ("netstat -an | awk '/ESTABLISHED/ {print $5}' | cut -d: -f1 | sort | uniq -c | sort -rn",
         "Rank remote IPs by active connections"),
...
def build_section_header(title, width):
...
def build_cmd_table(entries, width):
...
def draw_title_page(canvas, doc):
    """Title page: accent line at top, footer."""
    canvas.saveState()
    ...
    canvas.drawCentredString(PAGE_W / 2, 10 * mm, "The Linux Survival Kit  —  EdenX Labs")
    canvas.restoreState()

def draw_content_page(canvas, doc):
    """Content pages: accent line, page number, center divider."""
    canvas.saveState()
    # top accent
    canvas.setStrokeColor(ACCENT)
    ...
    canvas.line(center_x, MARGIN + 5*mm, center_x, PAGE_H - MARGIN - 2)
    # footer
    canvas.setFont("Helvetica", 6.5)
    canvas.setFillColor(TEXT_MED)
...
def build_pdf():
    output_path = "/home/claude/linux_cheatsheet_landscape.pdf"

    # Frame for title page (single column, full width)
    title_frame = Frame(
        MARGIN, MARGIN + 8*mm,
        PAGE_W - 2*MARGIN, PAGE_H - 2*MARGIN - 8*mm,
        id="title_frame"
    )

    # Frames for 2-column content pages
    left_frame = Frame(
        MARGIN, MARGIN + 8*mm,
        COL_W, PAGE_H - 2*MARGIN - 8*mm,
        id="left_col"
    )
    right_frame = Frame(
        MARGIN + COL_W + GUTTER, MARGIN + 8*mm,
        COL_W, PAGE_H - 2*MARGIN - 8*mm,
        id="right_col"
    )
    ...
    title_style = ParagraphStyle(
        "MainTitle", fontName="Helvetica-Bold", fontSize=28,
        textColor=TEXT_DARK, leading=34, alignment=TA_CENTER,
        spaceBefore=30, spaceAfter=2,
    )
    subtitle_style = ParagraphStyle(
        "MainSub", fontName="Helvetica", fontSize=12,
        textColor=TEXT_MED, leading=18, alignment=TA_CENTER,
        spaceAfter=4,
    )

    story.append(Spacer(1, 10))
    story.append(Paragraph("EdenX Labs presents", ParagraphStyle(
        "Presents", fontName="Helvetica", fontSize=10,
        textColor=HexColor("#888888"), leading=16, alignment=TA_CENTER,
        spaceBefore=0, spaceAfter=6,
    )))
    story.append(Paragraph("THE LINUX", title_style))
    story.append(Paragraph("SURVIVAL KIT", ParagraphStyle(
        "MainTitle2", parent=title_style, fontSize=32, spaceBefore=0, spaceAfter=8,
        textColor=HexColor("#0a58ca"),
    )))
    story.append(Paragraph("Commands You Actually Use", subtitle_style))
    story.append(Paragraph("31 sections  |  420+ commands  |  Landscape 2-column  |  Printable A4", ParagraphStyle(
        "MainSub2", parent=subtitle_style, fontSize=9, spaceAfter=14, textColor=HexColor("#888888"),
    )))

    # TOC
    toc_title_style = ParagraphStyle(
        "TOCTitle", fontName="Helvetica-Bold", fontSize=12,
        textColor=TEXT_DARK, leading=18, spaceBefore=6, spaceAfter=4,
        alignment=TA_CENTER,
    )
    story.append(Paragraph("TABLE OF CONTENTS", toc_title_style))

    toc_entries = [s[0] for s in sections if not s[0].startswith("__sub")]
    # 3-column TOC for landscape
    cols = 3
    per_col = (len(toc_entries) + cols - 1) // cols
    toc_cols = []
    for c in range(cols):
        start = c * per_col
        toc_cols.append(toc_entries[start:start + per_col])

    toc_style_p = ParagraphStyle(
        "TOCEntry", fontName="Helvetica", fontSize=7.5,
        textColor=TEXT_MED, leading=11,
    )
    toc_rows = []
    for i in range(per_col):
        ...
        for c in range(cols):
            if i < len(toc_cols[c]):
                row.append(Paragraph(toc_cols[c][i], toc_style_p))
            else:
                row.append("")
        toc_rows.append(row)

    toc_col_w = (full_w - 20) / cols
    toc_tbl = Table(toc_rows, colWidths=[toc_col_w] * cols)
    ...
    ref_title = ParagraphStyle(
        "RefTitle", fontName="Helvetica-Bold", fontSize=8.5,
        textColor=TEXT_DARK, leading=12, alignment=TA_CENTER, spaceAfter=2,
    )
    ref_style = ParagraphStyle(
        "Ref", fontName="Courier", fontSize=6.8,
        textColor=TEXT_MED, leading=9.5, alignment=TA_CENTER,
    )

    story.append(Spacer(1, 10))

    # Build 2x2 grid of reference boxes
    box_data = [
        ("FILE PERMISSIONS",
         "-rwxrw-r--  =  Type(-) | User(rwx) | Group(rw-) | Others(r--)<br/>"
         "r=4  w=2  x=1  |  chmod 755 = rwxr-xr-x  |  chmod 644 = rw-r--r--<br/>"
         "u=user  g=group  o=others  a=all  |  + add  - remove  = set exact"),
        ("CRON SCHEDULE",
         "min(0-59)  hour(0-23)  day(1-31)  month(1-12)  weekday(0-7)<br/>"
         "  *          *           *          *             *       command<br/>"
         "*/5 * * * *  (every 5 min)  |  0 9 * * 1-5  (weekdays 9 AM)"),
        ("FILE DESCRIPTORS &amp; REDIRECTION",
         "fd 0 = stdin  |  fd 1 = stdout  |  fd 2 = stderr<br/>"
         "&gt;  overwrite  |  &gt;&gt;  append  |  &lt;  input  |  |  pipe stdout<br/>"
         "2&gt;&amp;1  stderr-&gt;stdout  |  &amp;&gt;  both to file  |  /dev/null = discard"),
        ("CHAINING OPERATORS",
         "A &amp;&amp; B  run B if A ok  |  A || B  run B if A fails  |  A ; B  always<br/>"
         "A | B  pipe stdout  |  A |&amp; B  pipe stdout+stderr<br/>"
         "$(cmd)  capture output  |  $?  exit code  |  !!  repeat last cmd"),
    ]
    ...
    for i in range(0, len(box_data), 2):
        ...
        for j in range(2):
            if i + j < len(box_data):
                title_text, body_text = box_data[i + j]
                cell_content = [
                    Paragraph(title_text, ref_title),
                    Paragraph(body_text, ref_style),
                ]
    ...
    ref_col_w = (full_w - 10) / 2
    ref_tbl = Table(ref_rows, colWidths=[ref_col_w, ref_col_w])
    ...
    # ── Switch to 2-column template ─────────────────────────────────
    story.append(NextPageTemplate("content_pages"))
    ...
    for title, entries in sections:
        if title.startswith("__sub_"):
            ...
            story.append(build_cmd_table(entries, COL_W))
            story.append(Spacer(1, 3))
            continue

        if entries is None:
            top_sp = 4 if first_section else 8
            story.append(Spacer(1, top_sp))
            story.append(build_section_header(title, COL_W))
            story.append(Spacer(1, 3))
            first_section = False
            continue

        top_sp = 4 if first_section else 8
        header = build_section_header(title, COL_W)

        if len(entries) <= 8:
            # Short section: keep header + entire table together
            table = build_cmd_table(entries, COL_W)
            ...
            # Long section: keep header + first 5 rows together, rest flows
            head_entries = entries[:2]
            tail_entries = entries[2:]
            head_table = build_cmd_table(head_entries, COL_W)
            tail_table = build_cmd_table(tail_entries, COL_W)
            story.append(KeepTogether([
                Spacer(1, top_sp),
                header,
                Spacer(1, 3),
